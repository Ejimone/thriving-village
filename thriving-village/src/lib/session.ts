...
import { JWT_COOKIE, ROLE_COOKIE, NAME_COOKIE, type Role } from "./constants";

export type Session = { jwt: string; role: Role; name: string };
...
export async function getSession(): Promise<Session | null> {
  ...
  return { jwt, role, name: name || role };
}

export async function setSession(jwt: string, role: Role, name: string) {
  const store = await cookies();
  ...
  store.set(NAME_COOKIE, name, { httpOnly: false, secure, sameSite: "lax", path: "/", maxAge: MAX_AGE });
}
